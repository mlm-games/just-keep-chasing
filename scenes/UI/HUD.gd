...
func _ready() -> void:
	RunData.mito_energy_updated.connect(func(val): update_currency_label(val); update_progress_bar(val))
...
func update_progress_bar(new_amount: int) -> void:
	...
	var tween = create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD).set_pause_mode(Tween.TWEEN_PAUSE_PROCESS).set_ignore_time_scale()
