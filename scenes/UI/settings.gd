...
func _update_resolution_options_enabled():
	...
	elif GameState.video["fullscreen"]:
		%ResolutionButton.disabled = true
...
func audio_ready() -> void:
	%MainVolumeSlider.value = GameState.audio["Master"]
	%MusicSlider.value = GameState.audio["Music"]
	%SFXSlider.value = GameState.audio["SFX"]
...
func _on_borderless_button_toggled(toggled_on: bool) -> void:
	...
	GameState.video["borderless"] = toggled_on
